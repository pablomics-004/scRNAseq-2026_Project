...
## DEGs with FDR < 0.05
head(tt_2hr[(tt_2hr$adj.P.Val < 0.05) & (abs(tt_2hr$logFC) > 1), ], n = 5)
```
...
df <- as.data.frame(colData(rse_SRP048565_filtered)[, "sra_attribute.infection", drop = FALSE])
...
## DEGs with FDR < 0.05
head(tt_6hr[(tt_6hr$adj.P.Val < 0.05) & (abs(tt_6hr$logFC) > 1), ], n = 5)
```
...
head(tt_12hr[(tt_12hr$adj.P.Val < 0.05) & (abs(tt_12hr$logFC) > 1), ], n = 5)
```
...
volcanoplot(eb_results, coef = "Inf_vs_Ctrl_12hr", highlight = 3, names = gene_names[rownames(eb_results)])
```
